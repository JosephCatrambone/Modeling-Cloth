...
bl_info = {
    ...
    "version": (1, 0),
    "blender": (2, 80, 0),
    "location": "View3D > Modeling Cloth",
...
def get_last_object():
    ...
    cloths = [i for i in bpy.data.objects if
              i.modeling_cloth]  # so we can select an empty and keep the settings menu up
    ...
    for g in range(0, len(groups)):
        if groups[g] not in vg.keys():  # Don't create groups if there are already there
            ...
            vg[groups[g]].add(range(0, len(ob.data.vertices)), 0,
                              'ADD')  # This way we avoid resetting the weights for existing groups.
...
def generate_guide_mesh():
    ...
    verts = [[0.0, 0.0, 0.0], [-0.01, -0.01, 0.1], [-0.01, 0.01, 0.1], [0.01, -0.01, 0.1], [0.01, 0.01, 0.1],
             [-0.03, -0.03, 0.1], [-0.03, 0.03, 0.1], [0.03, 0.03, 0.1], [0.03, -0.03, 0.1], [-0.01, -0.01, 0.2],
             [-0.01, 0.01, 0.2], [0.01, -0.01, 0.2], [0.01, 0.01, 0.2]]
    edges = [[0, 5], [5, 6], [6, 7], [7, 8], [8, 5], [1, 2], [2, 4], [4, 3], [3, 1], [5, 1], [2, 6], [4, 7], [3, 8],
             [9, 10], [10, 12], [12, 11], [11, 9], [3, 11], [9, 1], [2, 10], [12, 4], [6, 0], [7, 0], [8, 0]]
    faces = [[0, 5, 6], [0, 6, 7], [0, 7, 8], [0, 8, 5], [1, 3, 11, 9], [1, 2, 6, 5], [2, 4, 7, 6], [4, 3, 8, 7],
             [3, 1, 5, 8], [12, 10, 9, 11], [4, 2, 10, 12], [3, 4, 12, 11], [2, 1, 9, 10]]
...
def create_instance(new=True):
    ...
    if new:
        ...
        cloth.virtual_springs = np.empty((0, 2),
                                         dtype=np.int32)  # so we can attach points to each other without regard to topology
    ...
    cloth.pin_bool = ~np.array([cloth.ob.vertex_groups['modeling_cloth_pin'].weight(i) for i in range(cloth.count)],
                               dtype=np.bool)
    ...
    if cloth.ob.modeling_cloth_handler_frame | cloth.ob.modeling_cloth_handler_scene:
        ...
        if not cloth.waiting:
            ...
            for x in range(cloth.ob.modeling_cloth_iterations):
                ...
                tiled_move = np.append(move, -move, axis=0)[
                                 cloth.unpinned] * mix  # * mix for stability: force multiplied by 1/number of springs
                ...
                if len(cloth.pin_list) > 0:
                    hook_co = np.array(
                        [cloth.ob.matrix_world.inverted() * i.matrix_world.to_translation() for i in cloth.hook_list])
...
def v_per_tri(co, tri_min, tri_max, idxer, tridexer, c_peat=None, t_peat=None):
    ...
    c_peat, t_peat = z_grid(co_z, tri_min[:, 2], tri_max[:, 2], subs, co_x, tri_min[:, 0], tri_max[:, 0], co_y,
                            tri_min[:, 1], tri_max[:, 1])
    if c_peat is None:
        return
    # X
...
def object_collide(cloth, object):
    ...
    if box_check:
        ...
        tris_in, tri_min, tri_max = triangle_bounds_check(tri_co, co1_min, co1_max, object.tridexer,
                                                          fudge)  # , object.ob.dimensions)
        ...
        if np.any(tris_in):
            ...
            if np.any(back_check):
                v_tris = v_per_tri(cloth.co[back_check], tri_min, tri_max, cloth.idxer[back_check],
                                   object.tridexer[tris_in])
                ...
                if v_tris is not None:
                    ...
                    in_margin = (d > -(inner_margin + outer_margin)) & (
                            d < 0)  # outer_margin) (we have offset outer margin)

                    # <<<--- Inside triangle check --->>>
                    # will overwrite in_margin:
                    cross_2 = object.cross_vecs[tris_in][tidx][in_margin]
                    inside_triangles(cross_2, vec2[in_margin], cloth.co, marginalized[tris_in], cidx, tidx, nor, ori,
                                     in_margin)
...
def self_collide(cloth):
    ...
    if v_tris is not None:
        ...
        if np.any(in_margin):
            ...
            c_move = ((nor[in_margin] * d_in[:, nax]) - (
                    nor[in_margin] * sign_margin[:, nax]))  # ) * -np.sign(d[in_margin])[:, nax]
...
def main(context, event):
    ...
    for obj, matrix in visible_objects_and_duplis():
        ...
        if hit is not None:
            ...
            verts = np.array([matrix * obj.data.shape_keys.key_blocks['modeling cloth key'].data[v].co for v in
                              obj.data.polygons[face_index].vertices])
            ...
            if best_obj is None or length_squared < best_length_squared:
                ...
                extra_data['latest_hit'] = matrix * obj.data.shape_keys.key_blocks['modeling cloth key'].data[
                    closest].co
...
def create_properties():
    ...
    bpy.types.Object.modeling_cloth_handler_scene = bpy.props.BoolProperty(name="Modeling Cloth Continuous Update",
                                                                           ...
                                                                           default=False,
                                                                           update=manage_continuous_handler)

    bpy.types.Object.modeling_cloth_handler_frame = bpy.props.BoolProperty(
        name="Modeling Cloth Handler Animation Update",
    ...
    bpy.types.Object.modeling_cloth_noise = bpy.props.FloatProperty(name="Modeling Cloth Noise",
                                                                    ...
                                                                    default=0.001, precision=4, min=0, max=1,
                                                                    update=refresh_noise)
    ...
    bpy.types.Object.modeling_cloth_noise_decay = bpy.props.FloatProperty(name="Modeling Cloth Noise Decay",
                                                                          ...
                                                                          default=0.99, precision=4, min=0,
                                                                          max=1)  # , update=refresh_noise_decay)
    ...
    bpy.types.Object.modeling_cloth_spring_force = bpy.props.FloatProperty(name="Modeling Cloth Spring Force",
                                                                           ...
                                                                           default=1.0, precision=4, min=0,
                                                                           max=2.5)  # , update=refresh_noise)
    ...
    bpy.types.Object.modeling_cloth_bend_stiff = bpy.props.FloatProperty(name="Modeling Cloth Bend Spring Force",
                                                                         ...
                                                                         default=0.0, precision=4, min=0, max=10,
                                                                         soft_max=1)  # , update=refresh_noise)
    # -------------------------->>>
    ...
    bpy.types.Object.modeling_cloth_wind_x = bpy.props.FloatProperty(name="Wind X",
                                                                     ...
                                                                     default=0, min=-10, max=10, soft_min=-1,
                                                                     soft_max=1)  # , update=refresh_noise_decay)
    ...
    bpy.types.Object.modeling_cloth_turbulence = bpy.props.FloatProperty(name="Wind Turbulence",
                                                                         ...
                                                                         default=0, min=0, max=10, soft_min=0,
                                                                         soft_max=1)  # , update=refresh_noise_decay)
    ...
    bpy.types.Object.modeling_cloth_self_collision_margin = bpy.props.FloatProperty(name="Margin",
                                                                                    ...
                                                                                    default=.08, precision=4, min=-1,
                                                                                    max=1, soft_min=0, soft_max=1)
    ...
    bpy.types.Object.modeling_cloth_object_collision = bpy.props.BoolProperty(name="Modeling Cloth Self Collsion",
                                                                              ...
                                                                              default=False,
                                                                              update=collision_object_update)
    ...
    bpy.types.Object.modeling_cloth_outer_margin = bpy.props.FloatProperty(name="Modeling Cloth Outer Margin",
                                                                           ...
                                                                           default=0.04, precision=4, min=0, max=100,
    ...
    bpy.types.Object.modeling_cloth_inner_margin = bpy.props.FloatProperty(name="Modeling Cloth Inner Margin",
                                                                           ...
                                                                           default=0.08, precision=4, min=0, max=100,
                                                                           soft_min=0, soft_max=1000)
    # ---------------------------->>>
    ...
    bpy.types.Object.modeling_cloth_grid_size = bpy.props.IntProperty(name="Modeling Cloth Grid Size",
                                                                      ...
                                                                      default=10, min=0, max=1000, soft_min=0,
                                                                      soft_max=1000)
...
class ModelingClothPanel(bpy.types.Panel):
    ...
    bl_region_type = 'UI'  # Was TOOLS
    bl_category = "ModCloth"
        ...
        if ob is not None:
            if ob.type == 'MESH':
                ...
                if ob.modeling_cloth:
                    ...
                    col.prop(ob, "modeling_cloth_iterations",
                             text="Iterations")  # , icon='OUTLINER_OB_LATTICE')
                    col.prop(ob, "modeling_cloth_spring_force",
                             text="Stiffness")  # , icon='OUTLINER_OB_LATTICE')
                    col.prop(ob, "modeling_cloth_push_springs",
                             text="Push Springs")  # , icon='OUTLINER_OB_LATTICE')
                    col.prop(ob, "modeling_cloth_bend_stiff",
                             text="Bend Springs")  # , icon='CURVE_NCURVE')
